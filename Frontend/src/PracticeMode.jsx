...
export default function PracticeMode() {
    ...
    // คำนวณ Progress สำหรับหลอดคะแนน
    ...
    return (
        <div style={{ minHeight: '100vh', backgroundColor: '#F3F4F6', padding: '30px 20px', fontFamily: '"Kanit", sans-serif' }}>
            ...
            <div className="main-container slide-up">
                        ...
                        {categories.length === 0 ? (
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '15px' }}>
                                {categories.map((category, index) => (
                                    <div 
                                        key={index} className="card-hover" onClick={() => handleSelectCategory(category)}
                                        ...
                                        <div style={{ display: 'flex', alignItems: 'center' }}>
                                            ...
                                            <h3 style={{ margin: 0, color: '#1F2937', fontSize: '18px', fontWeight: '500' }}>{category}</h3>
                                        </div>
                ...
                {/* 📍 หน้า 2: เลือกพาร์ทย่อย (อัปเดต UI แล้ว) */}
                {step === 'select_part' && (
                    <div className="fade-in">
                        {/* จับปุ่มกลับและหัวข้อมาเรียงในแนวนอนเดียวกันด้วย Flexbox */}
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '15px' }}>
                            {parts.map((part) => (
                                <div key={part.id} className="card-hover" onClick={() => handleSelectPart(part)} style={{ padding: '20px', borderRadius: '8px', cursor: 'pointer', display: 'flex', alignItems: 'center' }}>
                                    {/* เปลี่ยนไอคอนจากตกใจสีเหลือง เป็นไอคอนแฟ้มข้อสอบสีฟ้า */}
                ...
                {step === 'summary' && (
                    <div className="fade-in" style={{ textAlign: 'center', padding: '30px 0' }}>
                        <svg width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="#D69E2E" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" style={{ marginBottom: '20px' }}>
                            <path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path>
                            <polyline points="22 4 12 14.01 9 11.01"></polyline>
                        </svg>
                        
                        <h2 style={{ color: '#1A365D', margin: '0 0 10px 0', fontSize: '24px', fontWeight: '600' }}>เสร็จสิ้นการฝึกซ้อม</h2>
                        <p style={{ color: '#6B7280', margin: '0 0 30px 0', fontSize: '15px' }}>ระบบได้ทำการประเมินและสะสมคะแนนให้คุณเรียบร้อยแล้ว</p>
                        
                        <div style={{ background: '#F8FAFC', border: '1px solid #E2E8F0', borderRadius: '8px', padding: '30px', margin: '0 auto 35px auto', maxWidth: '350px' }}>
                            <div style={{ fontSize: '14px', color: '#4B5563', fontWeight: '500' }}>จำนวนข้อที่ตอบถูก</div>
                            <div style={{ fontSize: '56px', color: '#10B981', fontWeight: '600', margin: '10px 0', lineHeight: '1' }}>
                                {examHistory.filter(h => h.feedback.is_correct).length} <span style={{fontSize: '24px', color: '#9CA3AF'}}>/ 20</span>
                            </div>
                        </div>

                        <div style={{ display: 'flex', justifyContent: 'center', gap: '15px' }}>
                            <button onClick={() => { setStep('select_category'); setSelectedCategory(null); setSelectedPart(null); }} style={{ flex: 1, padding: '12px 20px', cursor: 'pointer', background: '#FFFFFF', color: '#4B5563', border: '1px solid #D1D5DB', borderRadius: '6px', fontSize: '15px', fontWeight: '500', transition: 'background 0.2s' }} onMouseOver={(e) => e.target.style.background = '#F3F4F6'} onMouseOut={(e) => e.target.style.background = '#FFFFFF'}>
                                กลับหน้าหลัก
                            </button>
